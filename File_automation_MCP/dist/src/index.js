...
const defaultDirectory = "D:\\GitHub\\March-Cohort\\File_system_MCP";
if (args.length === 0) {
    console.warn(`⚠️ No directory specified. Using default: ${defaultDirectory}`);
    args.push(defaultDirectory);
}
    ...
    try {
        ...
        switch (name) {
            ...
            case "edit_file": {
                ...
                const validPath = await validatePath(parsed.data.path);
                const result = await applyFileEdits(validPath, parsed.data.edits.map(edit => ({
